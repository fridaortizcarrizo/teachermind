...
export default function ProgressTracking() {
  ...
  const activeStudentId = selectedStudent || students[0]?.id || "";
  const student = students.find((s) => s.id === activeStudentId);

  const { data: studentGrammar = [] } = useGrammarTopics(activeStudentId || undefined);
  const { data: studentVocab = [] } = useVocabulary(activeStudentId || undefined);
  const { data: studentNotes = [] } = useProgressNotes(activeStudentId || undefined);
  const { data: studentLessons = [] } = useLessons(activeStudentId || undefined);

  const grammarData = studentGrammar.map((g) => ({ name: g.topic, times: g.times_worked, status: g.status }));
  const vocabByStatus = [
  ...
  if (loadingStudents) return <div className="max-w-5xl mx-auto space-y-6"><Skeleton className="h-12 w-64" /><Skeleton className="h-64 rounded-2xl" /></div>;
  ...
  if (students.length === 0) return (
    <div className="max-w-5xl mx-auto">
      ...
      <GlassCard variant="subtle" className="text-center py-12">
        <p className="text-muted-foreground">Add students to start tracking progress.</p>
      </GlassCard>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Select value={activeStudentId} onValueChange={setSelectedStudent}>
          <SelectTrigger className="w-48 bg-white/10 border-white/20 rounded-xl"><SelectValue /></SelectTrigger>
          <SelectContent>
        ...
        <GlassCard>
          ...
          {grammarData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={grammarData}><XAxis dataKey="name" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey="times" fill="hsl(15, 70%, 45%)" radius={[6, 6, 0, 0]} /></BarChart>
            </ResponsiveContainer>
          ...
          {vocabByStatus.length > 0 ? (
            <div className="flex items-center gap-8">
              <ResponsiveContainer width={160} height={160}>
                <PieChart><Pie data={vocabByStatus} dataKey="value" cx="50%" cy="50%" outerRadius={70} innerRadius={40}>{vocabByStatus.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip /></PieChart>
              </ResponsiveContainer>
